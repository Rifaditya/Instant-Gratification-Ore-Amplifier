...
> [!WARNING]
> **CONFIGURATION WARNING:** The global configuration file (and the ModMenu/YACL configuration screen) only defines default values for **NEWLY GENERATED WORLDS**. Settings for existing/active worlds must be adjusted in-game using the `/gamerule` command or the **Edit Game Rules** GUI screen. Changing values in the global config after a world has been created will not affect that world.
