...
## [1.3.20+26.3] - 2026-09-07

### Fixed
- **RepeatingPlacement Interface Compatibility ([BL-OREAMP-001] / [ERR-20260907-001])**: Retargeted `@ModifyExpressionValue` in `RepeatingPlacementMixin` from removed `getPositions()` to the `default modify()` method in the `RepeatingPlacement` interface, resolving an `InvalidMixinException` crash on world generation in Minecraft 26.3. Updated Loom `minecraft_version` to `26.3-pre-2` for dev/runtime jar alignment.

## [1.3.19+26.3] - 2026-09-05
